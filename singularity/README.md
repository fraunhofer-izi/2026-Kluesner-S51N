...
The Image is based on the Ubuntu 22.04 (R 4.5.1), see `./singularity/rstudio_server/recipe-4-5-1.def` for details
...
``` sh
$ sudo singularity build singularity-rstudio-4-5-1.sif recipe-4-5-1.def
```
...
``` sh
$ bash run.sh run singularity-rstudio-4-5-1.sif
```
