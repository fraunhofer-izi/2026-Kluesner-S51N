...
)
{
  ...
  if(category == "CD4" | category == "CD8") {
    ...
    msig_df = mir
    msig_df.gl = msig_df[grepl("global|states|Azizi", msig_df$TermID), ]
    msig_df = msig_df[grepl(category, msig_df$TermID), ]
    msig_df = rbind(msig_df.gl, msig_df)

    june.exh = c(
      "GNLY", "GZMB", "RBPJ", "AHI1", "IL2RA", "CTSW", "AFAP1L2", "PLS3", "CDK6",
      "KLRC2", "KLRB1", "KLRD1", "KLRC1", "CCL4", "CCL3", "PHLDA1", "CD9", "SOX4",
      "ID3", "SQLE", "NDFIP2", "TNFRSF18", "LYST", "TNFRSF9", "CSF1", "DUSP4",
      "SRGAP3","LAYN", "RGS16", "HAVCR2"
    )
    june.exh = data.frame(TermID = "Dysfunction", GeneID=  june.exh)
    msig_df = rbind(msig_df, june.exh)

    unique(msig_df$TermID)

    msig_df$TermID = gsub(paste0(category, "-"), "", msig_df$TermID)
    msig_df$TermID = gsub(" \\(Chu\\)", "", msig_df$TermID)
    msig_df$TermID = gsub(" \\(Cytopus\\)", "", msig_df$TermID)
    msig_df$TermID = gsub(" \\(Cytopus global\\)", "", msig_df$TermID)
    msig_df$TermID = gsub(" \\(Cell states\\)", "", msig_df$TermID)
    ...
    p = "data/signatures/combined_metabolic_pathways.csv"
    pathways <- SCPA::get_paths(p) %>% do.call("rbind", .)
    msig_list = split(
      x = pathways$Genes, f = pathways$Pathway
    )
...
ora_bubble = function(
  ...
  # gsea.df$richfactor = gsea.df$foldEnrichment

  gsea.df$pathway = gsub("GOBP_", "", gsea.df$pathway)
  gsea.df$pathway = gsub("KEGG_T_CELL_RECEPTOR", "KEGG_TCR", gsea.df$pathway)
  gsea.df$pathway = gsub("TCELL_RECEPTOR_TCR", "TCR", gsea.df$pathway)
  ...
  gsea.df$pathway = gsub(
    "ANTIGEN_PROCESSING_AND_PRESENTATION", "APAP", gsea.df$pathway
  )
  ...
  gsea.df$pathway = gsub("NATURAL_KILLER", "NK", gsea.df$pathway)
  gsea.df$pathway = gsub("POSITIVE_ALPHA_BETA_", "", gsea.df$pathway)
  gsea.df$pathway = gsub("POSITIVE", "POS.", gsea.df$pathway)
  gsea.df$pathway = gsub("NEGATIVE", "NEG.", gsea.df$pathway)
  ...
  if(!is.null(ftrs.list)) {
    ...
    for (i in 1:nrow(gsea.df)) {
      ...
      zscore = (length(ftrs.lfc[ftrs.lfc > 0]) -length(ftrs.lfc[ftrs.lfc < 0])) /
        sqrt(gs.ftrs)
    ...
    if(is.null(max.value)){
      ...
      if(quantile.filter == T) {
        thres = quantile(gsea.df$zscore, quantile.filter.thres)
    ...
    if(quantile.filter == T) {
      thres = quantile(gsea.df$fill, quantile.filter.thres)
      gsea.df$fill[gsea.df$fill > thres] = thres
    }
  }
  ...
  if(!is.null(ftrs.list)) {
    pl = pl + geom_point(
      ...
      scale_fill_scico(
        ...
        breaks = pretty_breaks(n = pretty.breaks)
      )
  } else {
    ...
    pl = pl + geom_point(
      ...
      scale_fill_gradientn(
        colors = scico(30, palette = bubble.col, direction = -1),
