...
fastqs = paste0(manifest$ho, "fastq/")
...
out.dir = paste0(manifest$ho, "cellranger/")
...
write_subscript = function(path, job_id, csv.out){
  ...
  # write(paste("#SBATCH --nodelist=ribnode[001-06]"), file = path, append = TRUE)
  write(paste("SBATCH --partition pbatch_all"), file = path, append = TRUE)
  write(paste("#SBATCH --exclude=ribnode[017,018,020]"), file = path, append = TRUE)
...
df = df[grepl("fastq.gz$", df$SAMPLE), ]
df = df[!grepl("I1|I2", df$SAMPLE), ]
df$SAMPLE = gsub("_S._.+", "", df$SAMPLE)
df$SOURCE = "Gene Expression"
