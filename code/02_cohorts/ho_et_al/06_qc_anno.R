...
output.file = paste0(manifest$ho$work, "seurat/02_seurat_pre.Rds")
output.file.2 = paste0(manifest$ho$work, "seurat/03_seurat_anno_t.Rds")

se.meta = readRDS(paste0(manifest$ho$work, "seurat/01_seurat_ori.Rds"))
...
ggsave2(
  filename="figures/ho_et_al/qc_pre_processing/stats_tech_per_cell.pdf",
...
# se.t = readRDS(output.file.2)
fltr = FetchData(
  se.t, vars = c("CD8B", "CD8A", "CD4CD8_BY_EXPRS"), layer = "counts"
)
r = (fltr$CD4CD8_BY_EXPRS == "CD4-CD8+") & (fltr$CD8A == 0 | fltr$CD8B == 0)
se.t = se.t[, !colnames(se.t) %in% rownames(fltr[r, ])]
