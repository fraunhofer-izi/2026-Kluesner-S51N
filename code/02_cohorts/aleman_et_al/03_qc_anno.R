...
output.file = paste0(manifest$aleman$work, "seurat/02_seurat_pre.Rds")
output.file.2 = paste0(manifest$aleman$work, "seurat/03_seurat_anno_t.Rds")

se.meta = readRDS(paste0(manifest$aleman$work, "seurat/01_seurat_ori.Rds"))
...
ggsave2(
  filename="figures/aleman_et_al/qc_pre_processing/stats_tech_per_cell.pdf",
    ...
    suppressMessages({
      ...
      obj.l = parallel::mclapply(obj.l, function(x) {
        x = scGate::scGate(
          ...
          output.col.name = "SCGATE", ncores = 10
        )
...
se.t = assign_vdj(
  obj = se.t,
  vdj = "vdj_t",
  batch = c(paste0(manifest$aleman$work, "cellranger")),
  present.bool = F
)
