...
fastqs = paste0(manifest$aleman$work, "fastq/")
...
out.dir = paste0(manifest$aleman$work, "cellranger/")
...
df = df %>% dplyr::mutate(
  SOURCE = dplyr::case_when(
    grepl("-GEX$", SAMPLE) ~ "Gene Expression",
    grepl("-TCR$", SAMPLE) ~ "VDJ-T"
  )
)

df$SAMPLE_SHORT = gsub("-GEX|-TCR", "", df$SAMPLE)
