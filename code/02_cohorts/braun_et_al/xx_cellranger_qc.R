...
Read_Metrics_10X_Multi <- function(
  ...
  if (dir.exists(paths = base_path) == FALSE) {
    cli_abort(
      message = "Directory: {.val {base_path}} specified by {.code base_path} does not exist."
    )
  }
  ...
  for (i in 1:length(x = lib_list)) {
    ...
    if (dir.exists(paths = full_directory_path) == FALSE) {
      cli_abort(
        message = "Full Directory does not exist {.val {full_directory_path}} was not found."
      )
    }
  }
  ...
  raw_data_list <- lapply(1:length(x = lib_list), function(x) {
    file_path <- file.path(
      base_path, lib_list[x], paste0(secondary_path, "/", lib_list[x], "/")
    )
    raw_data <- read.csv(
      file = paste0(file_path, "metrics_summary.csv"), stringsAsFactors = F
    )
    raw_data$Metric.Value = as.numeric(raw_data$Metric.Value)
    return(raw_data)
  })
...
metrics = Read_Metrics_10X_Multi(base_path = paste0(manifest$braun_rerun$cellranger))
...
ggsave2(
  plot_grid(a, b, c, ncol = 1, align = "vh", scale = .95),
  filename = "figures/braun_ent_al/secondary_analysis/gex.pdf",
...
ggsave2(
  ...
  filename = "figures/braun_et_al/secondary_analysis/vdj_tcell_1.pdf",
  units = "mm", width = 230, height = 180, scale = 1.1
)

ggsave2(
  plot_grid(c, d, e, ncol = 1, align = "vh", scale = .95),
  filename = "figures/braun_et_al/secondary_analysis/vdj_tcell_2.pdf",
